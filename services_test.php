...
$tblDemo->omitField("Notes");
	...
	return "<a href='services_test.php?idservice=$people' ><img width='25' height='25' src=/images/icon-people.png /></a> <a href='setlist.php?idservice=$people' ><img width='25' height='25' src=/images/list.png /></a> ";
	}
...
<div class='span7'>
		...
		if(isset($_GET['idservice'])){
			$id = $_GET['idservice'];
			echo "<div style='border:1px solid #dddddd; margin-bottom:20px;'>";
		
		echo "<h3>Rota information</h3>	";
				
	
	 $tblrota = new ajaxCRUD("Rota", "rota", "idrota", "");
	 #define relationship
     $tblrota->defineRelationship("role_idrole", "role", "idrole", "role_name");
    $tblrota->defineRelationship("users_id", "users", "id", "username");
    
       

    #i don't want to visually show the primary key in the table
    $tblrota->omitPrimaryKey();

    #the table fields have prefixes; i want to give the heading titles something more meaningful
    $tblrota->displayAs("role_idrole", "Role");
    $tblrota->displayAs("users_id", "Person");
 

        $tblrota->omitField("service_idservice");
	
	$tblrota->addWhereClause("WHERE service_idservice  = $id");
	
	
	$tblrota->showTable();
	echo "</div>";
	
		echo "<div style='border:1px solid #dddddd'>";
		
		echo "<h3>Songs for ###</h3>";
	echo "</div>";
